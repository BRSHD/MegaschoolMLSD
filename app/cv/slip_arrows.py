...
def detect_orthogonal_arrows(img_input, output_dir: str | None = None, debug: bool = False) -> List[dict]:
    ...
    def find_lines_by_direction(mask, direction="h"):
        ...
        for cnt in cnts:
            ...
            if (w if direction == "h" else h) < 10:
                continue
            if direction == "h":
                ends = [(x, y + h // 2), (x + w, y + h // 2)]
            else:
                ends = [(x + w // 2, y), (x + w // 2, y + h)]
        ...
        for seg in group:
            for ex, ey in seg["ends"]:
                ...
                if neighbor_count > 0:
                    is_internal = True
                if not is_internal:
                    external_ends.append((int(ex), int(ey)))
